...
bool fastEnvelope::seg_seg_intersection_2d(const std::array<Vector2, 2> &seg1, const std::array<Vector2, 2> &seg2, Scalar& t2){
    //assumptions:
...
fastEnvelope::Scalar fastEnvelope::p_seg_squared_dist_3d(const Vector3 &v, const Vector3 &a, const Vector3 &b){
    Vector3 av = v-a;
    Vector3 ab = b-a;
    if(av.dot(ab)<0)
...
bool fastEnvelope::seg_plane_intersection(const Vector3& p1, const Vector3& p2, const Vector3& a, const Vector3& n,
                                          Vector3& p, Scalar& d1) {
    Vector3 u = p2 - p1;
    Vector3 w = p1 - a;
...
int fastEnvelope::is_tri_tri_cutted(const std::array<Vector3, 3> &f_tri, const std::array<Vector3, 3> &f_tet,
...
bool fastEnvelope::is_tri_tri_cutted_2d(const std::array<Vector2, 3>& vs_tet, const std::array<Vector2, 3>& vs_tri) {
...
bool fastEnvelope::is_seg_tri_cutted_2d(const std::array<Vector2, 2> &seg, const std::array<Vector2, 3> &tri) {
...
bool fastEnvelope::is_p_inside_tri_2d(const Vector2& p, const std::array<Vector2, 3> &tri) {
    int cnt_pos = 0;
    int cnt_neg = 0;
...
int fastEnvelope::get_t(const Vector3 &p0, const Vector3 &p1, const Vector3 &p2) {
...
fastEnvelope::Vector2 fastEnvelope::to_2d(const Vector3 &p, int t) {
    return Vector2(p[(t + 1) % 3], p[(t + 2) % 3]);
}

bool fastEnvelope::is_crossing(int s1, int s2) {
...
int fastEnvelope::is_tri_tri_cutted(const Vector3& p1, const Vector3& p2, const Vector3& p3,
...
int fastEnvelope::is_tri_tri_cutted_hint(const Vector3& p1, const Vector3& p2, const Vector3& p3,
...
void fastEnvelope::get_bbox_face(const Vector3& p0, const Vector3& p1, const Vector3& p2,
...
void fastEnvelope::get_bbox_tet(const Vector3& p0, const Vector3& p1, const Vector3& p2, const Vector3& p3,
        Vector3& min, Vector3& max, Scalar eps) {
    min = p0;
    max = p0;
    for (int j = 0; j < 3; j++) {
...
bool fastEnvelope::is_bbox_intersected(const Vector3& min1, const Vector3& max1, const Vector3& min2, const Vector3& max2) {
    for (int j = 0; j < 3; j++) {
        if (min1[j] > max2[j])
            return false;
        if (max1[j] < min2[j])
...
bool fastEnvelope::is_tri_inside_tet(const std::array<Vector3, 3>& ps,
...
bool fastEnvelope::is_point_inside_tet(const Vector3& p, const Vector3& p0t, const Vector3& p1t, const Vector3& p2t, const Vector3& p3t) {
    int cnt_pos = 0;
    int cnt_neg = 0;
