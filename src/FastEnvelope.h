...
namespace fastEnvelope {
	...
	private:
		...
		static int orient3D_LPI_filtered_multiprecision(
			...
			int prec = 10000;
			T a11;
			a11.value->_mp_prec = prec;
			a11=(px - qx);

			T a12;
			a12.value->_mp_prec = prec;
			
			a12=(py - qy);

			T a13;
			a13.value->_mp_prec = prec;
			a13=(pz - qz);

			T a21;
			a21.value->_mp_prec = prec;
			a21=(sx - rx);

			T a22;
			a22.value->_mp_prec = prec;
			a22=(sy - ry);

			T a23;
			a23.value->_mp_prec = prec;
			a23=(sz - rz);

			T a31;
			a31.value->_mp_prec = prec;
			a31=(tx - rx);

			T a32;
			a32.value->_mp_prec = prec;
			a32=(ty - ry);

			T a33;
			a33.value->_mp_prec = prec;
			a33=(tz - rz);

			T a2233;
			a2233.value->_mp_prec = prec;
			a2233=((a22 * a33) - (a23 * a32));

			T a2133;
			a2133.value->_mp_prec = prec;
			a2133=((a21 * a33) - (a23 * a31));

			T a2132;
			a2132.value->_mp_prec = prec;
			a2132=((a21 * a32) - (a22 * a31));

			T d;
			d.value->_mp_prec = prec;
			...
			T px_rx;
			px_rx.value->_mp_prec = prec;
			px_rx=(px - rx);

			T py_ry;
			py_ry.value->_mp_prec = prec;
			py_ry=(py - ry);

			T pz_rz;
			pz_rz.value->_mp_prec = prec;
			pz_rz=(pz - rz);

			T n;
			n.value->_mp_prec = prec;
			n=((((py_ry)* a2133) - ((px_rx)* a2233)) - ((pz_rz)* a2132));



			T px_cx;
			px_cx.value->_mp_prec = prec;
			px_cx=(px - cx);

			T py_cy;
			py_cy.value->_mp_prec = prec;
			py_cy=(py - cy);

			T pz_cz;
			pz_cz.value->_mp_prec = prec;
			pz_cz=(pz - cz);



			T d11;
			d11.value->_mp_prec = prec;
			d11=((d * px_cx) + (a11 * n));

			T d21;
			d21.value->_mp_prec = prec;
			d21=(ax - cx);

			T d31;
			d31.value->_mp_prec = prec;
			d31=(bx - cx);

			T d12;
			d12.value->_mp_prec = prec;
			d12=((d * py_cy) + (a12 * n));

			T d22;
			d22.value->_mp_prec = prec;
			d22=(ay - cy);

			T d32;
			d32.value->_mp_prec = prec;
			d32=(by - cy);

			T d13;
			d13.value->_mp_prec = prec;
			d13=((d * pz_cz) + (a13 * n));

			T d23;
			d23.value->_mp_prec = prec;
			d23=(az - cz);

			T d33;
			d33.value->_mp_prec = prec;
			d33=(bz - cz);



			T d2233;
			d2233.value->_mp_prec = prec;
			d2233=(d22 * d33);

			T d2332;
			d2332.value->_mp_prec = prec;
			d2332=(d23 * d32);

			T d2133;
			d2133.value->_mp_prec = prec;
			d2133=(d21 * d33);

			T d2331;
			d2331.value->_mp_prec = prec;
			d2331=(d23 * d31);

			T d2132;
			d2132.value->_mp_prec = prec;
			d2132=(d21 * d32);

			T d2231;
			d2231.value->_mp_prec = prec;
			d2231=(d22 * d31);


			T det;
			det.value->_mp_prec = prec;
