...
void RealModelEnvelopeTest() {
	const std::string input_surface_path = "D:\\vs\\float project\\data\\buser_head.stl";
	...
	const Scalar bbd = 1000;
	FastEnvelope::BoxGeneration(vertices, faces, envprism, bbd);
	std::cout << "face size " << faces.size() << " envsize: " << envprism.size() << std::endl;
	std::ofstream fout;
	...
	fout.close();
	//std::array<Vector3, 3> face = { Vector3(1.3,3,0),Vector3(2,3,4.7),Vector3(3,5,0) };
	//std::array<Vector3, 3> face = { Vector3(1.3,3,0),Vector3(2,3,4.7),Vector3(3,4.5,0) };
	std::array<Vector3, 3> face = {{ vertices[faces[1][0]],vertices[faces[1][1]],vertices[faces[1][2]] }};
	Parameters params;
	std::vector<std::array<Vector3, 2>> list;
	std::vector<int> inumber;
	CornerList(params, envprism, list);
	BondingBoxIntersectionFinding(params, face, list, inumber);
	std::vector<std::array<Vector3, 12>> interprism;
	std::vector<std::array<Vector3, 12>> interprism1;
	for (int i = 0; i < inumber.size(); i++) {
		interprism.push_back(envprism[inumber[i]]);
	}
	std::cout << "mark before envelope test:" << " interprism size: " << interprism.size() << std::endl;
	for (int i = 0; i < inumber.size(); i++) {
		//std::cout << "inumber " << inumber[i] << std::endl;
	}
	//interprism1.push_back(interprism[1]);
	bool Out = FastEnvelope::FastEnvelopeTest(face, interprism);
	std::cout << "if the triangle is out of the prism  " << Out << std::endl;

	fout.open("D:\\vs\\float project\\data\\testtriangle.txt");
	for (int i = 0; i < 3; i++) {
		fout << face[i][0] << "              " << face[i][1] << "             " << face[i][2] << endl;
	}
	fout.close();



	fout.open("D:\\vs\\float project\\data\\vertices.txt");
	for (int i = 0; i < vertices.size(); i++) {
		fout << vertices[i][0] << "              " << vertices[i][1] << "             " << vertices[i][2] << endl;
	}
	fout.close();
	fout.open("D:\\vs\\float project\\data\\faces.txt");
	for (int i = 0; i < faces.size(); i++) {
		fout << faces[i][0] << "               " << faces[i][1] << "              " << faces[i][2] << endl;
	}
	fout.close();
	...
	//std::fout.close();                  
	//std::cout << "vertices: "<<192 <<"\n"<< vertices[192] << std::endl;
...
void comparison() {
	...
	for (int i = 0; i < fn; i++) {
		...
		pos1[i] = FastEnvelope::FastEnvelopeTestImplicit(triangle[i], interenvprism);
		pos2[i] = FastEnvelope::FastEnvelopeTestTemp(triangle[i], interenvprism);
		time5 = time5 + timer4.getElapsedTimeInSec();//function time


	}
	time1 = time1 + timer2.getElapsedTimeInSec();

	std::cout << "TEST ONE FINISHED  " << std::endl;
	/////////////////////////////////

		//////////////////
	std::vector<int> inumber2;
	...
	int mk = 36259;
	inumber2.clear();
	BondingBoxIntersectionFinding(params, triangle[mk], list, inumber2);
	interenvprism.clear();
	for (int j = 0; j < inumber2.size(); j++) {
		interenvprism.push_back(envprism[inumber2[j]]);
	}
	std::cout << "interprism size: " << interenvprism.size() << std::endl;
	...
	fout.open("D:\\vs\\float project\\data\\testtriangle.txt");
	for (int i = 0; i < 3; i++) {
		fout << std::setprecision(17) << triangle[mk][i][0] << "              " << triangle[mk][i][1] << "             " << triangle[mk][i][2] << endl;
	}
	fout.close();
...
void orient3D_LPITest() {


	std::array<Vector3, 2> l;

	std::array<Vector3, 3> p1;
	std::array<Vector3, 3> p2;
	l[0] = { 0,0,3 };
	l[1] = { 0, 0, -1 };
	p1[0] = { 0, 0, 0 };
	p1[1] = { 1, 0, 0 };
	...
	p2[1] = { 1, 0, 1 };
	p2[2] = { 0, 1, 1 };
	double ax, ay, az, bx, by, bz, cx, cy, cz, a11, a12, a13, a21, a22, a23, a31, a32, a33, px_rx, py_ry, pz_rz, d, n;
	int jm = 0, ori;
	bool inter = FastEnvelope::is_seg_facet_intersection(l[0][0], l[0][1],l[0][2],
		l[1][0], l[1][1], l[1][2],
		p1[0][0], p1[0][1], p1[0][2],
		p1[1][0], p1[1][1], p1[1][2],
		p1[2][0], p1[2][1], p1[2][2],
		a11, a12, a13, a21, a22, a23, a31, a32, a33, px_rx, py_ry, pz_rz, d, n);
	//std::cout <<"intersect or not "<< inter << std::endl;
	ori = FastEnvelope::orient3D_LPI(
		l[0][0], l[0][1], l[0][2], p2[0][0], p2[0][1], p2[0][2], p2[1][0], p2[1][1], p2[1][2], p2[2][0], p2[2][1], p2[2][2],
		a11, a12, a13, a21, a22, a23, a31, a32, a33, px_rx, py_ry, pz_rz, d, n);
	//std::cout << "n and d: " << n << " " << d << std::endl;
	//std::cout << "ori " << ori << std::endl;
	bool inter1 = FastEnvelope::is_seg_facet_intersection_Wang(l[0], l[1], p1, d, n);
	std::cout << "intersect or not " << inter1 << std::endl;
	std::cout << "n and d: " << n << " " << d << std::endl;
	int ori1 = FastEnvelope::Orient_LPI_Wang(l[0], l[1], d, n, p2);
	std::cout << "ori " << ori1 << std::endl;
	ori1 = Predicates::orient_3d(p2[0],p2[1],p2[2], l[0] + (l[1] - l[0])*n / d);
	std::cout << "ori in old way " << ori1 << std::endl;
	/*bool inter = FastEnvelope::orient3D_LPI_Original(l[0][0], l[0][1], l[0][2],
		l[1][0], l[1][1], l[1][2],
