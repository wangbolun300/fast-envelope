	...
	for (int i = 0; i < prismindex.size(); i++)
	{
		...
		if (prismindex[i] < prism_size)
		{
			...
			for (int j = 0; j < p_facenumber; j++)
			{
				...
				e00 = envprism[prismindex[i]][p_face[j][0]][0];
				e01 = envprism[prismindex[i]][p_face[j][0]][1];
				e02 = envprism[prismindex[i]][p_face[j][0]][2];
				e10 = envprism[prismindex[i]][p_face[j][1]][0];
				e11 = envprism[prismindex[i]][p_face[j][1]][1];
				e12 = envprism[prismindex[i]][p_face[j][1]][2];
				e20 = envprism[prismindex[i]][p_face[j][2]][0];
				e21 = envprism[prismindex[i]][p_face[j][2]][1];
				e22 = envprism[prismindex[i]][p_face[j][2]][2];
				// timer.start();
		...
		else
		{
			...
			for (int j = 0; j < c_facenumber; j++)
			{
				...
				e00 = envcubic[prismindex[i] - prism_size][c_face[j][0]][0];
				e01 = envcubic[prismindex[i] - prism_size][c_face[j][0]][1];
				e02 = envcubic[prismindex[i] - prism_size][c_face[j][0]][2];
				e10 = envcubic[prismindex[i] - prism_size][c_face[j][1]][0];
				e11 = envcubic[prismindex[i] - prism_size][c_face[j][1]][1];
				e12 = envcubic[prismindex[i] - prism_size][c_face[j][1]][2];
				e20 = envcubic[prismindex[i] - prism_size][c_face[j][2]][0];
				e21 = envcubic[prismindex[i] - prism_size][c_face[j][2]][1];
				e22 = envcubic[prismindex[i] - prism_size][c_face[j][2]][2];
				// timer.start();
				ori = orient3D_LPI_post_exact(s, s00, s01, s02,
											  e00, e01, e02, e10, e11, e12,
											  e20, e21, e22);
		...
		for (int k = 0; k < recompute.size(); k++)
		{
			...
			if (in1 < prism_size)
			{
				for (int j = 0; j < recompute[k].FACES.size(); j++)
				{
					...
					e00 = envprism[in1][p_face[in2][0]][0];
					e01 = envprism[in1][p_face[in2][0]][1];
					e02 = envprism[in1][p_face[in2][0]][2];
					e10 = envprism[in1][p_face[in2][1]][0];
					e11 = envprism[in1][p_face[in2][1]][1];
					e12 = envprism[in1][p_face[in2][1]][2];
					e20 = envprism[in1][p_face[in2][2]][0];
					e21 = envprism[in1][p_face[in2][2]][1];
					e22 = envprism[in1][p_face[in2][2]][2];
					// timer.start();
					ori = orient3D_LPI_post_exact(s, s00, s01, s02,
												  e00, e01, e02, e10, e11, e12,
												  e20, e21, e22);
			...
			else
			{
				for (int j = 0; j < recompute[k].FACES.size(); j++)
				{
					...
					e00 = envcubic[in1 - prism_size][c_face[in2][0]][0];
					e01 = envcubic[in1 - prism_size][c_face[in2][0]][1];
					e02 = envcubic[in1 - prism_size][c_face[in2][0]][2];
					e10 = envcubic[in1 - prism_size][c_face[in2][1]][0];
					e11 = envcubic[in1 - prism_size][c_face[in2][1]][1];
					e12 = envcubic[in1 - prism_size][c_face[in2][1]][2];
					e20 = envcubic[in1 - prism_size][c_face[in2][2]][0];
					e21 = envcubic[in1 - prism_size][c_face[in2][2]][1];
					e22 = envcubic[in1 - prism_size][c_face[in2][2]][2];
					// timer.start();
					ori = orient3D_LPI_post_exact(s, s00, s01, s02,
												  e00, e01, e02, e10, e11, e12,
												  e20, e21, e22);
...
int FastEnvelope::Implicit_Tri_Facet_Facet_interpoint_Out_Prism_pure_multiprecision(const DATA_TPI &datatpi, const std::array<Vector3, 3> &triangle, const std::vector<unsigned int> &prismindex) const
{
	...
	// time_multi += timer.getElapsedTimeInSec();
	if (premulti == false)
		return 2; //means have parallel facets
