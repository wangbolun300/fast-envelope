...
void AABB::init_envelope_boxes_recursive(
    const std::vector<std::array<Vector3, 2>> &cornerlist, int node_index,
    int b, int e) {
  assert(b != e);
  ...
  for (int c = 0; c < 3; ++c) {
    boxlist[node_index][0][c] =
        std::min<Scalar>(boxlist[childl][0][c], boxlist[childr][0][c]);
    boxlist[node_index][1][c] =
        std::max<Scalar>(boxlist[childl][1][c], boxlist[childr][1][c]);
  }
}
...
void AABB::segment_search_bbd_recursive(const Vector3 &seg0,
                                        const Vector3 &seg1,
                                        std::vector<unsigned int> &list, int n,
                                        int b, int e) const {
  assert(e != b);
...
void AABB::bbd_searching_recursive(const Vector3 &bbd0, const Vector3 &bbd1,
                                   std::vector<unsigned int> &list, int n,
                                   int b, int e) const {
  assert(e != b);
...
int AABB::envelope_max_node_index(int node_index, int b, int e) {
  ...
  int childl = 2 * node_index;
  int childr = 2 * node_index + 1;
  return std::max<int>(envelope_max_node_index(childl, b, m),
...
void AABB::init(const std::vector<std::array<Vector3, 2>> &cornerlist) {
  ...
  boxlist.resize(envelope_max_node_index(1, 0, n_corners) +
...
bool AABB::is_segment_cut_bounding_box(const Vector3 &seg0, const Vector3 &seg1,
                                       int index) const {
  const auto &bmin = boxlist[index][0];
  const auto &bmax = boxlist[index][1];
  Scalar min[3], max[3];
...
bool AABB::is_bbd_cut_bounding_box(const Vector3 &bbd0, const Vector3 &bbd1,
                                   int index) const {
  const auto &bmin = boxlist[index][0];
  const auto &bmax = boxlist[index][1];
